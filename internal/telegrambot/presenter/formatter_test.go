...
func TestFormatSearchResultBuildsKeyboard(t *testing.T) {
	...
	if !strings.Contains(text, "<b>Found 12 books. Page 2/3.</b>") {
		t.Fatalf("unexpected text: %s", text)
	}
	if !strings.Contains(text, `<a href="/download/t1">War &amp; Peace</a>`) {
		t.Fatalf("expected HTML link, got %s", text)
	}
...
func TestFormatSearchResultEscapesHTML(t *testing.T) {
	...
	result := &corepresenter.PresenterResult{
		SearchResult: &corepresenter.SearchResult{
			...
			Total:   1,
			Books: []corepresenter.BookDTO{
				{ID: "1", Title: `Book <One> & Co`, FullAuthors: `A > B & C`, DownloadURL: `https://example.test/?q=a&b=c`},
			},
		},
	...
	if !strings.Contains(text, `href="https://example.test/?q=a&amp;b=c"`) {
		t.Fatalf("expected escaped url, got %s", text)
	}
}
...
func TestFormatErrorEscapesHTML(t *testing.T) {
	...
	if !strings.Contains(text, `<code>tg-1&lt;2&gt;</code>`) {
		t.Fatalf("expected escaped trace, got %s", text)
	}
}
