...
type Mapping struct {
    RealSHA1     string
    RealPath     string // container/filename
    CreatedAt    time.Time
    ExpiresAt    time.Time
    Accessed     int
...
type Mapper struct {
    config  *gateway.MapperConfig
    mu      sync.RWMutex
    tokens  map[string]*Mapping
    stopCh  chan struct{}
}

func NewMapper(cfg *gateway.MapperConfig) *Mapper {
    m := &Mapper{
        config: cfg,
...
// GenerateToken создаёт новый токен для SHA1
func (m *Mapper) GenerateToken(sha1, path string) (string, error) {
    ...
    token := base64.RawURLEncoding.EncodeToString(bytes) // 43 символа
    ...
    logger.Info("", "mapper: generated token",
        "sha1", sha1,
        "token_len", len(token),
        "ttl", m.config.TTL)
...
// Resolve получает реальный SHA1 по токену
...
// Revoke удаляет токен (например, после использования)
...
// Stats возвращает статистику маппера
